  ...
  data() {
    return {
      ...
      rules: {
        ...
        hospital: [
          { required: true, message: this.message.pleaseInputHospitalName, trigger: "blur" }
        ],
        time: [{ required: true, message: this.message.pleaseInputExamineTime, trigger: "change" }]
      },
      fileList: [],
  ...
  methods: {
    ...
    fetchDetail: async function() {
      ...
      if (res.data.status_code == 200) {
        ...
        this.form.oxygen_saturation_value =
          res.data.data.oxygen_saturation_value;
