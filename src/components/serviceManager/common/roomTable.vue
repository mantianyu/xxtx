<template>
	<div class="table">
		    ...
		    <el-table-column
		      ...
		      label="房间名称">
		      	<template slot-scope="scope">
			        <router-link :to="{ path: '/fwgl/cwsz/bedt/'+lid+'/'+fid+'/'+scope.row.id,query:{buildname:buildname,roomname:scope.row.name}}">
	              		{{scope.row.name}}
	            	</router-link>
		        </template>
		    </el-table-column>
		...
		<v-eroom :roomID="roomID" :directions="directions" :nursingType="careType" :nurseProject="nurseProject" :bedType="bedType"></v-eroom>
		<v-aroom :lid="lid" :fid="fid" :directions="directions" :nursingType="careType" :nurseProject="nurseProject" :bedType="bedType"></v-aroom>
	</div>
</template>

<script>
import vEroom from "./roomEdit.vue";
import vAroom from "./addRoom.vue";
...
export default {
  ...
  created: function() {
    ...
    this.floornum = ">" + this.$route.query.floornum + this.message.floor;
  },
  components: {
    vEroom,
    vAroom
  ...
  methods: {
    ...
    fetchList: async function(val) {
      this.loading = true;
      let url = this.api.roomlist + "?page=" + val;
      var data = {
        fid: this.$route.params.fid
      };
      try {
    ...
    delMore() {
      if (this.multipleSelection.length == 0) {
        this.$alert(this.message.pleaseChoiceRoom, "", {
          confirmButtonText: this.message.confirm,
          callback: action => {}
        });
      } else {
    ...
    showEedit(scope) {
      this.$store.dispatch("roomEditTrue");
      this.roomID = scope.id;
    },
    clean() {
      this.roomID = 0;
    },
    showAdd() {
      this.$store.dispatch("addRoomTrue");
      this.roomID = 0;
    },
