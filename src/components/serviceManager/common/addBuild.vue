<template>
    <div class="addBuild" v-show="addBuild">
                ...
                <el-form :rules="rules" ref="form" :model="form" label-width="120px">
                  <div style="display: flex;flex-wrap: wrap;">
                    <el-form-item label="楼栋编号:" prop="code">
                        <el-input v-model="form.code" type="number" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="楼栋名称:" prop="name">
                        <el-input v-model="form.name" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="楼层数量:" prop="floor_count">
                        <el-input v-model.number="form.floor_count" type="number" style="width: 200px;"></el-input>
                    </el-form-item>
...
export default {
  props: ["directions", "nursingType", "bedType"],
  data() {
    ...
    var checkRoom = (rule, value, callback) => {
      if (this.form.room_count > 0 && !value) {
        callback(
          new Error(
            rule.message
              ? rule.message
    ...
    var vaDirection = (rule, value, callback) => {
      if (this.form.floor_count && !this.form.direction) {
    ...
    return {
      ...
      rules: {
        code: [{ required: true, message: this.message.pleaseInputBuildingNumber, trigger: "blur" }],
        name: [{ required: true, message: this.message.pleaseInputBuildingName, trigger: "blur" }],
        floor_count: [
          {
            validator: checkRoom,
            message: this.message.mustAddFloorNumber,
            trigger: "blur,change"
          },
          {
            validator: checkBed,
          ...
          },
          {
            ...
            attrName: this.message.floorNumber,
            trigger: "blur",
            message: this.message.floorNumberMustBeNum
          }
        ],
        room_count: [
          {
  ...
  methods: {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          var data = {
            code: this.form.code,
            name: this.form.name,
            floor_count: this.form.floor_count,
    ...
    hideEditFalse() {
      this.$store.dispatch("addBuildFalse");
      this.$refs["form"].resetFields();
    },
    saveBuild: async function(data) {
      let url = this.api.dobuilding;
      try {
