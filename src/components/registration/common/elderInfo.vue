<template>
    <div>
            ...
            <el-form :rules="rules" ref="form" :model="form" label-width="120px" class="demo-ruleForm">
                ...
                <div style="display: flex;flex-wrap: wrap;">
                    ...
                    <el-form-item label="家庭人口:" prop="family_member_count">
                        <el-input v-bind:disabled="!editInfo" type="number" v-model="form.family_member_count" style="width: 200px;" min="0"></el-input>
                    </el-form-item>
                </div>
        ...
        <v-relation :erid="erid" :elderId="elderId" :relation="relation" :status="status"></v-relation>
    </div>
</template>
...
export default {
	...
	data() {
		...
		let numberCheck = (rule, value, callback) => {
			if (value === '') {
				callback(new Error(this.message.pleaseInputFamilyPopulation));
			} else if (value < 0) {
		...
		return {
			...
			rules: {
				...
				birthday: [{ required: true, message: this.message.pleaseChoiceElderBir, trigger: 'change' }],
				gender: [
					{
						required: true,
				...
				marital_status: [{ required: true, message: this.message.pleaseChoiceMarryCondi, trigger: 'change' }],
				nationality: [{ required: true, message: this.message.pleaseChoiceNation, trigger: 'change' }],
				blood_group: [{ required: true, message: this.message.pleaseChoiceBloodType, trigger: 'change' }],
				political_status: [{ required: true, message: this.message.pleaseChoicePolitice, trigger: 'change' }],
				profession: [{ required: true, message: this.message.pleaseChoiceRetireJob, trigger: 'change' }],
				is_resident: [{ required: true, message: this.message.pleaseChoiceWhetherLocal, trigger: 'change' }],
				is_ingoing_resident: [
					{ required: true, message: this.message.pleaseChoiceWhetherNonlocal, trigger: 'change' },
				],
				living_status: [{ required: true, message: this.message.pleaseChoiceOriLive, trigger: 'change' }],
				family_member_count: [
					{
						required: true,
				...
				home_address: [{ required: true, message: this.message.pleaseInputHomeAddress, trigger: 'blur' }],
				census_register: [
					{ required: true, message: this.message.pleaseInputDomicileAddress, trigger: 'blur' },
				],
				elder_type: [{ required: true, message: this.message.pleaseChoicePersonType, trigger: 'change' }],
				religion: [{ required: true, message: this.message.pleaseChoiceReligiom, trigger: 'change' }],
				hobbies: [{ required: true, message: this.message.pleaseInputHobbies, trigger: 'blur' }],
				psychologies: [{ required: true, message: this.message.pleaseInoutPsychologies, trigger: 'blur' }],
				eating_habits: [{ required: true, message: this.message.pleaseInputEatHabit, trigger: 'blur' }],
				education: [{ required: true, message: this.message.pleaseChoiceEduDegree, trigger: 'change' }],
				labour_capacity: [
					{ required: true, message: this.message.pleaseChoiceLaborAbility, trigger: 'change' },
				],
				income_source: [{ required: true, message: this.message.pleaseChoiceEcoSource, trigger: 'change' }],
				medical_security: [{ required: true, message: this.message.pleaseChoiceMedicare, trigger: 'change' }],
				medicare_card: [{ required: true, message: this.message.pleaseInputMedicalCard, trigger: 'blur' }],
				self_care: [{ required: true, message: this.message.pleaseChoiceSelfAbility, trigger: 'change' }],
				address: [{ required: true, message: this.message.pleaseInputHomeAddress, trigger: 'blur' }],
			},
			int0: 0,
			int1: 1,
			int2: 2,
