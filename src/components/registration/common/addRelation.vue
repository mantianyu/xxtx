	        		...
	        		<div style="display: flex; flex-wrap: wrap;">
	        			<el-form-item label="亲属姓名:" prop="ername">
					  	...
					  	<el-form-item label="联系电话:" prop="telphone">
					    	<el-input v-model="form.telphone" type="number" style="width: 200px;"></el-input>
					  	</el-form-item>
...
export default {
	props: ['erid', 'elderId', 'relation', 'status'],
	data() {
		...
		return {
			...
			rules: {
				ername: [{ required: true, message: this.message.pleaseInputKinName, trigger: 'blur' }],
				telphone: [{ required: true, message: this.message.pleaseInputPhone, trigger: 'blur' }],
				idcard: [{ required: false, validator: validateNumber, trigger: 'blur' }],
			},
			...
			try {
				...
				if (res.data.status_code == 200) {
					if (this.status === 1) {
						...
						this.$parent.JdrzqsManageList(this.elderId, 1);
					}
					this.bLoading = false;
					this.hideEditFalse();
				} else {
